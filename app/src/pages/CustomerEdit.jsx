...
import { INTEREST_LABEL, today, useStore } from '../store'
import { nextFollowup } from '../lib/coaching'
import { Stars, TopBar } from '../components/ui'
...
export default function CustomerEdit() {
  ...
  const [form, setForm] = useState({
    ...
    interest: c?.interest || 2, family: c?.family || '', memo: c?.memo || '', lastContact: c?.lastContact || '', birthday: c?.birthday || '',
  })
  ...
  const save = () => {
    ...
    if (c) {
      dispatch({ type: 'customer.update', id: c.id, data: { ...data, nextFollowup: nextFollowup({ ...c, ...data }) } })
      nav(`/customers/${c.id}`, { replace: true })
    } else {
      const base = { ...data, createdAt: today() }
      dispatch({ type: 'customer.add', data: { ...base, nextFollowup: nextFollowup(base) } })

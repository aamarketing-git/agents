...
import { INTEREST_LABEL, fmtDate, useStore } from '../store'
import { followupStatus } from '../lib/coaching'
import { Empty, TopBar } from '../components/ui'

/* 고객 목록 : 연락시점 · 관심도 · 팔로업 */
const FILTERS = [['all', '전체'], ['due', '연락할 분'], ['hot', '관심 높음'], ['new', '최근 등록']]
            ...
            return (
              <button key={c.id} className="list-item" onClick={() => nav(`/customers/${c.id}`)}>
                ...
                <div className="main">
                  ...
                  <div className="meta">{INTEREST_LABEL[c.interest || 0] || ''}{c.lastContact ? ` · 마지막 ${fmtDate(c.lastContact)}` : ' · 아직 연락 전'}</div>
                </div>

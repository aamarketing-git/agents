...
길이: ${p.length || '적당히'}
포함할 내용: ${p.points || '없음'}`,
  coach: (p) => `아래는 주인의 실제 고객·일정 데이터입니다. 질문이 고객·일정·관리 대상에 관한 것이면 반드시 이 데이터의 실제 이름과 날짜로 답하고, 없는 사실은 만들지 마세요. 제품·사업·SNS·자기관리 질문이면 성장 코치로서 답하세요. 300자 내외, 마지막에 오늘 할 행동 1가지.
${p.context || '(데이터 없음)'}
